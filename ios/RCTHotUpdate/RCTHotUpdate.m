...
//自定义
static NSString *_mainInstancePath = @"";
static NSString *keyUpdateInfo = @"";
...
+ (NSURL *)bundleURL
{
    NSURL *url = [RCTHotUpdate bundleURLForDownload];
    if (url) {
        return url;
    }else{
        return [RCTHotUpdate binaryBundleURL:_mainInstancePath];
    }
    
    
}
+ (NSURL *)bundleURLForResource:(NSString *)resourceName{
    _mainInstancePath = resourceName;
    keyUpdateInfo = [NSString stringWithFormat:@"%@_HOTUPDATE_INFO_KEY",resourceName];
    ...
    [RCTHotUpdateDownloader download:updateUrl savePath:zipFilePath progressHandler:^(long long receivedBytes, long long totalBytes) {
        ...
        if (error) {
            [self.bridge.eventDispatcher sendAppEventWithName:EVENT_PROGRESS_DOWNLOAD
                                                         body:@{
                                                                PARAM_PROGRESS_HASHNAME:hashName,
                                                                PARAM_PROGRESS_RECEIVED:[NSNumber numberWithLongLong:-1],
                                                                PARAM_PROGRESS_TOTAL:[NSNumber numberWithLongLong:1]
                                                                }];
            callback(error);
                    }
        else {
                    ...
                    if (error) {
                        ...
                        switch (type) {
                            case HotUpdateTypePatchFromPackage:
                            {
                                ...
                                NSString *bundleOrigin = [[RCTHotUpdate binaryBundleURL:_mainInstancePath] path];
...
{
    if ([resourceName isEqualToString:@""]) {
        resourceName = @"main";
    }
    NSURL *url = [[NSBundle mainBundle] URLForResource:resourceName withExtension:@"jsbundle"];
    NSLog(@"mainBundlePath=%@",[NSBundle mainBundle].bundlePath);
    return url;
}
